...
var UPRIGHTBtnArray=[];
var RIGHTBtnArray=[];
var DOWNRIGHTBtnArray=[];
var DOWNBtnArray=[];
var DOWNLEFTBtnArray=[];
var LEFTBtnArray=[];
var UPLEFTBtnArray=[];

var Nearest_eye_button;
var Swiping=false;
var SwipeCenterX;
var SwipeCenterY;
...
var c = document.getElementById("VisualHint_canvas");
...
var preTimeStamp = 0.0;
 ...
 var VHC = document.getElementById("VisualHint_canvas");
var VHCxt = VHC.getContext("2d");
// smooth pursuit
...
socket.on('eyemove', (x, y, ts) => {
    ...
    // and the reason.
    let magicScale =0.68; //surface pro should be 0.8
    // closeEye = false;
    UserState(ts);
...
function changePos(eyeX, eyeY) {
    ...
    if(!EyeStay(eyeX, eyeY) &&Swiping==false){
        RemoveColor();
        UPBtnArray=[];
        UPRIGHTBtnArray=[];
        RIGHTBtnArray=[];
    ...
    NewClearDirArray(eyeX,eyeY)
    console.log("Down: "+DOWNBtnArray+"DownRight"+DOWNRIGHTBtnArray+"DownLeft"+ DOWNLEFTBtnArray+" Right: "+RIGHTBtnArray)
    
    VHCxt.setTransform(1,0,0,1,0,0)
    //VHCxt.clearRect(0, 0, 1400, 1000);
    VHCxt.fillStyle="#FFFFFF"
    VHCxt.beginPath();
    VHCxt.fillRect(0, 0, server_width , server_height);
    VHCxt.clearRect(0, 0, server_width , server_height);
    VHCxt.closePath();
   
    //console.log("UPRIGHT: "+ UPRIGHTBtnArray +"UPRIGHT: "+ UPRIGHTBtnArray +)
    clearTimeout(LeaveTimer);
    ...
    //ClearDirArray(neighborhood)

     if(Swiping==false){
    Color(eyeX,eyeY);
    }
    else{
    Color(SwipeCenterX,SwipeCenterY);
    ...
    var NearestDis_eye_button=10000000;
    for (var k = 0; k < 9; k++) {
        ...
        if (overlap(btn, eyeX, eyeY)) {
            ...
            if(NearestDis_eye_button>curr_dist){
                NearestDis_eye_button=curr_dist
                Nearest_eye_button=i
            }
            if (curr_dist < dist[j]) {
                candidate[j] = i;
                dist[j] = curr_dist;
    ...
    console.log("Nearst : "+Nearest_eye_button)
    var NearbtnmidX = $(buttons[Nearest_eye_button]).offset().left + 0.5*buttons[Nearest_eye_button].offsetWidth;
    var NearbtnmidY = $(buttons[Nearest_eye_button]).offset().top + 0.5*buttons[Nearest_eye_button].offsetHeight;
    ViusalHintNear(eyeX,eyeY,NearbtnmidX,NearbtnmidY)
    ...
    for (var k = 0; k < 9; k++) {
        ...
        if (type === 'swipe'){
            if(Swiping==false){
            var btnmidX = $(btn).offset().left + 0.5*btn.offsetWidth;
            var btnmidY = $(btn).offset().top + 0.5*btn.offsetHeight;
            var vectorX=(btnmidX-eyeX);
            var vectorY=-(btnmidY-eyeY);
            
            var theta=ComputeTheta(vectorX,vectorY)*180/Math.PI;
            
            if(overlap(btn, eyeX, eyeY)){
                if(theta<0){
                    ASSIGNBTN(360+theta,i) 
                }
                else{
                ASSIGNBTN(theta,i) 
            }
...
function showTarget() {
    ...
    for (var cnt = 0; cnt < DISTRACT - 5;) {
        ...
        //var rand = Math.floor(Math.random() * 10 * COL_NUM);
...
function enableSwipe() {
    ...
    manager.add([swipe,singletap]);


    //listening tap 
    manager.on('singletap', (e) => {
        buttons[Nearest_eye_button].click()
        console.log("Tap!!");
         Swiping = false
        touchnow=false
    });
    ...
    manager.on('swipe', (e) => {
        ...
        let swipeangle=-e.angle;
        if(swipeangle<0){swipeangle=swipeangle+360}
        let dir = getSwipeDirection(hammerDir);
        
        swipeAndUnlock(dir);
        RemoveColor();
        
        swipeclick(swipeangle);
        console.log("I swipe: "+swipeangle);
...
function EyeStay(x, y) {
    ...
    for (var i = 0; i < 10; i++) {
        //if ((EyeXave - EyeStayX[i]) * (EyeXave - EyeStayX[i]) + (EyeYave - EyeStayY[i]) * (EyeYave - EyeStayY[i]) > BTN_SIZE * BTN_SIZE) {
           if ((EyeXave - EyeStayX[i]) * (EyeXave - EyeStayX[i]) + (EyeYave - EyeStayY[i]) * (EyeYave - EyeStayY[i]) > 113 * 113) {
    ...
    EyeStayTimeEnd = Date.now();
    if (EyeStayTimeEnd - EyeStayTimeStart > 150) {
...
function ComputeTheta(vectorX,vectorY){

    if(vectorX>0){
        return Math.atan(vectorY/vectorX)
    }
    else if(vectorX<0 && vectorY>=0){
        return Math.atan(vectorY/vectorX)+Math.PI
    }
    else if(vectorX<0 && vectorY<0){
         return Math.atan(vectorY/vectorX)-Math.PI
    }
 ...
 function ASSIGNBTN(theta,i){
    if(theta>22.5&&theta<=67.5){
        if(MyFind(UPRIGHTBtnArray,i)==false){UPRIGHTBtnArray.push(i)} 
    }
    else if(theta>67.5&&theta<=112.5){
        if(MyFind(UPBtnArray,i)==false){UPBtnArray.push(i)}
    }
    else if(theta>112.5&&theta<=157.5){
        if(MyFind(UPLEFTBtnArray,i)==false){UPLEFTBtnArray.push(i)}
      
    }
     else if(theta>157.5&&theta<=202.5){
        if(MyFind(LEFTBtnArray,i)==false){LEFTBtnArray.push(i)}
    }
    else if(theta>202.5&&theta<=247.5){
        if(MyFind(DOWNLEFTBtnArray,i)==false){DOWNLEFTBtnArray.push(i)} 
    }
    else if(theta>247.5&&theta<=292.5){
        if(MyFind(DOWNBtnArray,i)==false){DOWNBtnArray.push(i)}
       
    }
     else if(theta>292.5&&theta<=337.5){
        if(MyFind(DOWNRIGHTBtnArray,i)==false){DOWNRIGHTBtnArray.push(i)}
        
    }
     else if((theta>337.5&&theta<=360 )||(theta>=0.0&&theta<=22.5)){
        if(MyFind(RIGHTBtnArray,i)==false){RIGHTBtnArray.push(i)}
        
    }

 }
 /*
 function ClearDirArray(neighborhood){
    var temparray=[];
    for(i=0;i<UPRIGHTBtnArray.length;i++){

        var temp=UPRIGHTBtnArray.pop();
        if(temp!=null){

        if(MyFind(neighborhood,temp)==true){temparray.push(i)}}
    }
    UPRIGHTBtnArray=temparray;
    temparray=[];

    for(i=0;i<UPBtnArray.length;i++){
        var temp=UPBtnArray.pop();
         if(temp!=null){
        if(MyFind(neighborhood,temp)==true){temparray.push(i)}}
    }
    UPBtnArray=temparray;
    temparray=[];

    for(i=0;i<UPLEFTBtnArray.length;i++){
        var temp=UPLEFTBtnArray.pop();
         if(temp!=null){
        if(MyFind(neighborhood,temp)==true){temparray.push(i)}}
    }
    UPLEFTBtnArray=temparray;
    //console.log(UPLEFTBtnArray)
    temparray=[];

    for(i=0;i<LEFTBtnArray.length;i++){
        var temp=LEFTBtnArray.pop();
         if(temp!=null){
        if(MyFind(neighborhood,temp)==true){temparray.push(i)}}
    }
    LEFTBtnArray=temparray;
    temparray=[];

     for(i=0;i<DOWNLEFTBtnArray.length;i++){
        var temp=DOWNLEFTBtnArray.pop();
         if(temp!=null){
        if(MyFind(neighborhood,temp)==true){temparray.push(i)}}
    }
    DOWNLEFTBtnArray=temparray;
    temparray=[];
    
  for(i=0;i<DOWNBtnArray.length;i++){
        var temp=DOWNBtnArray.pop();
         if(temp!=null){
        if(MyFind(neighborhood,temp)==true){temparray.push(i)}}
    }
    DOWNBtnArray=temparray;
    temparray=[];
    
  for(i=0;i<DOWNRIGHTBtnArray.length;i++){
        var temp=DOWNRIGHTBtnArray.pop();
         if(temp!=null){
        if(MyFind(neighborhood,temp)==true){temparray.push(i)}}
    }
    DOWNRIGHTBtnArray=temparray;
    
    temparray=[];

    for(i=0;i<RIGHTBtnArray.length;i++){
        var temp=RIGHTBtnArray.pop();
         if(temp!=null){
        if(MyFind(neighborhood,temp)==true){temparray.push(i)}}
    }
    RIGHTBtnArray=temparray;
    temparray=[];
 ...
 */
 ...
 function NewClearDirArray(EyeX,EyeY){
    var temparray=[];
    for(i=0;i<UPRIGHTBtnArray.length;i++){
        ...
        var BtnX = $(buttons[temp]).offset().left + 0.5 * buttons[temp].offsetWidth;
        var BtnY = $(buttons[temp]).offset().top + 0.5 * buttons[temp].offsetHeight;
        var newdist=Math.sqrt((BtnX-EyeX)*(BtnX-EyeX)+(BtnY-EyeY)*(BtnY-EyeY))
        if(newdist<113){temparray.push(temp)}
        }
    }
...
function swipeclick(theta){

     if(theta>22.5&&theta<=67.5){
        if(UPRIGHTBtnArray.length>1){
            Swiping = true
            MultiTarget(theta,UPRIGHTBtnArray)
        }
        else{
            if( buttons[UPRIGHTBtnArray[0]]!=null){
                Swiping=false
                buttons[UPRIGHTBtnArray[0]].click()
                touchnow=false
             }
        }
       console.log("UPRIGHT: "+ UPRIGHTBtnArray)
    }
    else if(theta>67.5&&theta<=112.5){
     if(UPBtnArray.length>1){
         Swiping = true
            MultiTarget(theta,UPBtnArray)
        }
        else{
            if( buttons[UPBtnArray[0]]!=null){
                Swiping=false
                buttons[UPBtnArray[0]].click()
                touchnow=false
             }
        }
      console.log("UP: "+ UPBtnArray)
    }
    else if(theta>112.5&&theta<=157.5){
         if(UPLEFTBtnArray.length>1){
             Swiping = true
            MultiTarget(theta,UPLEFTBtnArray)
        }
        else{
            if( buttons[UPLEFTBtnArray[0]]!=null){

                Swiping=false
                buttons[UPLEFTBtnArray[0]].click()
                touchnow=false
             }
        }
       console.log("UPLEFT: "+ UPLEFTBtnArray)
    }
     else if(theta>157.5&&theta<=202.5){
         if(LEFTBtnArray.length>1){
             Swiping = true
            MultiTarget(theta,LEFTBtnArray)
        }
        else{
            if( buttons[LEFTBtnArray[0]]!=null){
                Swiping=false
                buttons[LEFTBtnArray[0]].click()
                touchnow=false
             }
        }
       console.log("LEFT: "+ LEFTBtnArray)
    }

    else if(theta>202.5&&theta<=247.5){
         if(DOWNLEFTBtnArray.length>1){
             Swiping = true
            MultiTarget(theta,DOWNLEFTBtnArray)
        }
        else{
            if( buttons[DOWNLEFTBtnArray[0]]!=null){
                Swiping=false
                buttons[DOWNLEFTBtnArray[0]].click()
                touchnow=false
             }
        }
       console.log("DOWNLEFT: "+ DOWNLEFTBtnArray)
    }
    else if(theta>247.5&&theta<=292.5){
        if(DOWNBtnArray.length>1){
             Swiping = true
            MultiTarget(theta,DOWNBtnArray)
        }
        else{
            if( buttons[DOWNBtnArray[0]]!=null){
                Swiping=false
                buttons[DOWNBtnArray[0]].click()
                touchnow=false
             }
        }
       console.log("DOWN: "+ DOWNBtnArray)
    }
     else if(theta>292.5&&theta<=337.5){
         if(DOWNRIGHTBtnArray.length>1){
             Swiping = true
            MultiTarget(theta,DOWNRIGHTBtnArray)
        }
        else{
            if( buttons[DOWNRIGHTBtnArray[0]]!=null){
                Swiping=false
                buttons[DOWNRIGHTBtnArray[0]].click()
                touchnow=false
             }
        }
       console.log("DOWNRIGHT: "+ DOWNRIGHTBtnArray)
    }
     else if((theta>337.5&&theta<=360 )||(theta>=0.0&&theta<=22.5)){
         if(RIGHTBtnArray.length>1){
             Swiping = true
            MultiTarget(theta,RIGHTBtnArray)
        }
        else{
            if( buttons[RIGHTBtnArray[0]]!=null){
                Swiping=false
                buttons[RIGHTBtnArray[0]].click()
                touchnow=false
             }
        }
       console.log("RIGHT: "+ RIGHTBtnArray)
    }
}
...
function Color(eyeX,eyeY){


        for(var i=0;i<UPRIGHTBtnArray.length;i++){
            //buttons[UPRIGHTBtnArray[i]].css('background-color','#111111');
            //$(buttons[UPRIGHTBtnArray[i]]).addClass('UpRight');
            
            var BtnX = $(buttons[UPRIGHTBtnArray[i]]).offset().left + 0.5 * buttons[UPRIGHTBtnArray[i]].offsetWidth;
            var BtnY = $(buttons[UPRIGHTBtnArray[i]]).offset().top + 0.5 * buttons[UPRIGHTBtnArray[i]].offsetHeight;
            ViusalHint(eyeX,eyeY,BtnX,BtnY)
        }
         for(var i=0;i<UPBtnArray.length;i++){
            //buttons[UPBtnArray[i]].css('background-color','#222222')
            //$(buttons[UPBtnArray[i]]).addClass('Up');
            var BtnX = $(buttons[UPBtnArray[i]]).offset().left + 0.5 * buttons[UPBtnArray[i]].offsetWidth;
            var BtnY = $(buttons[UPBtnArray[i]]).offset().top + 0.5 * buttons[UPBtnArray[i]].offsetHeight;
            ViusalHint(eyeX,eyeY,BtnX,BtnY)
         ...
         for(var i=0;i< UPLEFTBtnArray.length;i++){
           // buttons[UPLEFTBtnArray[i]].css('background-color','#333333')
             //$(buttons[UPLEFTBtnArray[i]]).addClass('UpLeft');
            var BtnX = $(buttons[UPLEFTBtnArray[i]]).offset().left + 0.5 * buttons[UPLEFTBtnArray[i]].offsetWidth;
            var BtnY = $(buttons[UPLEFTBtnArray[i]]).offset().top + 0.5 * buttons[UPLEFTBtnArray[i]].offsetHeight;
            ViusalHint(eyeX,eyeY,BtnX,BtnY)

        }
     
         for(var i=0;i<LEFTBtnArray.length;i++){
            //buttons[LEFTBtnArray[i]].css('background-color','#444444')
             //$(buttons[LEFTBtnArray[i]]).addClass('Left');
            var BtnX = $(buttons[LEFTBtnArray[i]]).offset().left + 0.5 * buttons[LEFTBtnArray[i]].offsetWidth;
            var BtnY = $(buttons[LEFTBtnArray[i]]).offset().top + 0.5 * buttons[LEFTBtnArray[i]].offsetHeight;
            ViusalHint(eyeX,eyeY,BtnX,BtnY)

        }
    

         for(var i=0;i< DOWNLEFTBtnArray.length;i++){
           // buttons[ DOWNLEFTBtnArray[i]].css('background-color','#555555')
             //$(buttons[ DOWNLEFTBtnArray[i]]).addClass('DownLeft');
            var BtnX = $(buttons[ DOWNLEFTBtnArray[i]]).offset().left + 0.5 * buttons[ DOWNLEFTBtnArray[i]].offsetWidth;
            var BtnY = $(buttons[ DOWNLEFTBtnArray[i]]).offset().top + 0.5 *buttons[ DOWNLEFTBtnArray[i]].offsetHeight;
            ViusalHint(eyeX,eyeY,BtnX,BtnY)

        }
     
   
         for(var i=0;i< DOWNBtnArray.length;i++){
           // buttons[DOWNBtnArray[i]].css('background-color','#666666')
             //$(buttons[DOWNBtnArray[i]]).addClass('Down');
            var BtnX = $(buttons[DOWNBtnArray[i]]).offset().left + 0.5 * buttons[DOWNBtnArray[i]].offsetWidth;
            var BtnY = $(buttons[DOWNBtnArray[i]]).offset().top + 0.5 * buttons[DOWNBtnArray[i]].offsetHeight;
            ViusalHint(eyeX,eyeY,BtnX,BtnY)

        }
    
         for(var i=0;i< DOWNRIGHTBtnArray.length;i++){
            //buttons[DOWNRIGHTBtnArray[i]].css('background-color','#777777')
             //$(buttons[DOWNRIGHTBtnArray[i]]).addClass('DownRight');
            var BtnX = $(buttons[DOWNRIGHTBtnArray[i]]).offset().left + 0.5 * buttons[DOWNRIGHTBtnArray[i]].offsetWidth;
            var BtnY = $(buttons[DOWNRIGHTBtnArray[i]]).offset().top + 0.5 * buttons[DOWNRIGHTBtnArray[i]].offsetHeight;
            ViusalHint(eyeX,eyeY,BtnX,BtnY)

        }
    
         for(var i=0;i<RIGHTBtnArray.length;i++){
            //buttons[RIGHTBtnArray[i]].css('background-color','#888888')
             //$(buttons[RIGHTBtnArray[i]]).addClass('Right');
            var BtnX = $(buttons[RIGHTBtnArray[i]]).offset().left + 0.5 * buttons[RIGHTBtnArray[i]].offsetWidth;
            var BtnY = $(buttons[RIGHTBtnArray[i]]).offset().top + 0.5 * buttons[RIGHTBtnArray[i]].offsetHeight;
            ViusalHint(eyeX,eyeY,BtnX,BtnY)
...
function RemoveColor(){
        ...
        for(var i=0;i<UPRIGHTBtnArray.length;i++){
            //buttons[UPRIGHTBtnArray[i]].css('background-color','#111111');
            $(buttons[UPRIGHTBtnArray[i]]).removeClass('UpRight');
        }
         for(var i=0;i<UPBtnArray.length;i++){
            //buttons[UPBtnArray[i]].css('background-color','#222222')
            $(buttons[UPBtnArray[i]]).removeClass('Up');
        }
     
         for(var i=0;i<UPLEFTBtnArray.length;i++){
           // buttons[UPLEFTBtnArray[i]].css('background-color','#333333')
             $(buttons[UPLEFTBtnArray[i]]).removeClass('UpLeft');
        }
     
         for(var i=0;i<LEFTBtnArray.length;i++){
            //buttons[LEFTBtnArray[i]].css('background-color','#444444')
             $(buttons[LEFTBtnArray[i]]).removeClass('Left');
        }
    
   
         for(var i=0;i<DOWNLEFTBtnArray.length;i++){
           // buttons[ DOWNLEFTBtnArray[i]].css('background-color','#555555')
             $(buttons[ DOWNLEFTBtnArray[i]]).removeClass('DownLeft');
        }
     
   
         for(var i=0;i<DOWNBtnArray.length;i++){
           // buttons[DOWNBtnArray[i]].css('background-color','#666666')
             $(buttons[DOWNBtnArray[i]]).removeClass('Down');
        }
    
         for(var i=0;i<DOWNRIGHTBtnArray.length;i++){
            //buttons[DOWNRIGHTBtnArray[i]].css('background-color','#777777')
             $(buttons[DOWNRIGHTBtnArray[i]]).removeClass('DownRight');
        }
    
         for(var i=0;i<RIGHTBtnArray.length;i++){
            //buttons[RIGHTBtnArray[i]].css('background-color','#888888')
             $(buttons[RIGHTBtnArray[i]]).removeClass('Right');
        }
}

function cross(eyeX,eyeY){
        //VHCxt.clearRect(0, 0, 1400, 1000);
        //VHCxt.fillStyle = "#FF2345";
        //VHCxt.moveTo(eyeX,eyeY);
        //VHCxt.lineTo(0, 200);
        //VHCxt.moveTo(eyeX,eyeY);
        //VHCxt.lineTo(200, 0);
...
function ViusalHint(X1,Y1,X2,Y2) {
    ...
    arrowX=X2+botX;
    arrowY=Y2+botY;
    VHCxt.lineTo(arrowX,arrowY)
    VHCxt.stroke();
        
    /*
    var theTheta=ComputeTheta((X2-X1),(Y2-Y1))*180/Math.PI;
    if(theTheta<0){theTheta=theTheta+360}
    var start=theTheta-10
    var end=theTheta+10
    if(start<0){start=start+360}


    console.log(theTheta)
    VHCxt.moveTo(X1,Y1)
    VHCxt.arc(X1,Y1,113,start*Math.PI/180,end*Math.PI/180,false)
    VHCxt.fillStyle = "rgb(0,255,1,0.1)";
    VHCxt.fill()
    */    
...
function MultiTarget(theta,CandidateArray){

    var SumBtnX=0.0
    var SumBtnY=0.0
    for(var i=0;i<CandidateArray.length;i++){
                //buttons[UPRIGHTBtnArray[i]].click()
            SumBtnX += $(buttons[CandidateArray[i]]).offset().left + 0.5 * buttons[CandidateArray[i]].offsetWidth;
            SumBtnY += $(buttons[CandidateArray[i]]).offset().top + 0.5 * buttons[CandidateArray[i]].offsetHeight;
        }
            SwipeCenterX=SumBtnX/CandidateArray.length
            SwipeCenterY=SumBtnY/CandidateArray.length

    var tempCandidateArray=CandidateArray
    UPBtnArray=[];
    UPRIGHTBtnArray=[];
    RIGHTBtnArray=[];
    DOWNRIGHTBtnArray=[];
    ...
    for(var i=0;i<tempCandidateArray.length;i++){
     var btnmidX = $(buttons[tempCandidateArray[i]]).offset().left + 0.5*buttons[tempCandidateArray[i]].offsetWidth;
            var btnmidY = $(buttons[tempCandidateArray[i]]).offset().top + 0.5*buttons[tempCandidateArray[i]].offsetHeight;
            var vectorX=(btnmidX-SwipeCenterX);
            var vectorY=-(btnmidY-SwipeCenterY);
            
            var theta=ComputeTheta(vectorX,vectorY)*180/Math.PI;

            if(theta<0){
                    ASSIGNBTN(360+theta,tempCandidateArray[i]) 
                }
                else{
                    ASSIGNBTN(theta,tempCandidateArray[i]) 
            }
...
function ViusalHintNear(X1,Y1,X2,Y2) {
    
    VHCxt.moveTo(X2,Y2)
    VHCxt.arc(X2,Y2,20,0,2*Math.PI,false)
    VHCxt.fillStyle = "rgb(0,255,255,0.7)";
    VHCxt.fill()
    ...
    console.log(theTheta)
    VHCxt.moveTo(X1,Y1)
    VHCxt.arc(X1,Y1,Dist,start*Math.PI/180,end*Math.PI/180,false)
    VHCxt.fillStyle = "rgb(0,255,1,0.3)";
    VHCxt.fill()
     */
